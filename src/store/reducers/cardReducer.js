...
export const delete_card_product = createAsyncThunk(
    ...
    async(cardId, { rejectWithValue, fulfillWithValue }) => {
        try {
            const {data} = await api.delete(`/home/product/delete-card-product/${cardId}`)
            // console.log(data);
            return fulfillWithValue(data)
        } catch (error) {
            return rejectWithValue(error.response.data)
...
export const cardReducer = createSlice({
    ...
    extraReducers : (builder) => {
        ...
        .addCase(add_to_wishlist.rejected, (state, { payload }) => {
            state.errorMessage = payload?.error || 'Erreur lors de l\'ajout à la wishlist'; 
        })
        .addCase(add_to_wishlist.fulfilled, (state, { payload }) => {
         state.successMessage = payload.message;
         // Le count sera mis à jour par get_wishlist_products
        })
        .addCase(get_wishlist_products.fulfilled, (state, { payload }) => {
         state.wishlist = payload.wishlist || [];
         state.wishlist_count = payload.wishlist ? payload.wishlist.length : 0;
        })
